...
import { Calendar, BookOpen, FileSpreadsheet, ClipboardList, Clock, CheckCircle2, TrendingUp, ArrowRight, AlertCircle, Heart, Users, UserCog } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      if (isAdmin) {
        // Admin: count students and teachers
        ...
        setStats(prev => ({
          ...
          totalGuru: teacherCount || 0,
        }));
      } else {
        // For siswa/guru, fetch attendance stats
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().split('T')[0];
        const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString().split('T')[0];

        // Note: attendance now uses student_id from students table
  ...
  const adminMenuItems = [
    {
      title: 'Kelola Siswa',
      description: 'Tambah, edit, dan hapus data siswa',
      icon: Users,
      url: '/admin/siswa',
      iconBg: 'bg-primary',
    },
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-1">
              ...
              <CardDescription className="text-primary-foreground/60 flex items-center gap-2">
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-foreground/10 text-primary-foreground">
                  {isAdmin ? 'Administrator' : isGuru ? 'Guru' : 'Siswa'}
                </span>
                {isSiswa && user?.class_name && (
                  <span className="text-primary-foreground/50">• {user.class_name}</span>
                )}
                {isGuru && user?.subject && (
                  <span className="text-primary-foreground/50">• {user.subject}</span>
                )}
              </CardDescription>
            </div>
      ...
      {/* Quick Stats */}
      <div className={`grid gap-4 ${isAdmin ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-4'}`}>
        {statsCards.map((stat) => (
      ...
      <Card className="shadow-card bg-muted/50">
        <CardContent className="p-5">
          <p className="text-sm text-muted-foreground">
            {isAdmin ? (
              <>
                <span className="font-medium text-foreground">Panel Admin:</span> Kelola data siswa dan guru di sini.
              </>
            ) : (
              <>
                <span className="font-medium text-foreground">Statistik Bulan Ini:</span> Total {loading ? '-' : stats.totalHadir + stats.totalIzin + stats.totalSakit + stats.totalAlpha} hari tercatat.
              </>
            )}
          </p>
        </CardContent>
